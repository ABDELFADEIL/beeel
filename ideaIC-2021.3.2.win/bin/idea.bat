...
SET BITS=64
SET "_USER_JRE64_FILE=%APPDATA%\JetBrains\IdeaIC2021.3\idea%BITS%.exe.jdk"
SET BITS=
SET "_USER_JRE_FILE=%APPDATA%\JetBrains\IdeaIC2021.3\idea%BITS%.exe.jdk"
IF "%JRE%" == "" (
  SET _JRE_CANDIDATE=
...
IF "%VM_OPTIONS_FILE%" == "" (
  ...
  ) ELSE IF EXIST "%APPDATA%\JetBrains\IdeaIC2021.3\idea%BITS%.exe.vmoptions" (
    SET "USER_VM_OPTIONS_FILE=%APPDATA%\JetBrains\IdeaIC2021.3\idea%BITS%.exe.vmoptions"
  )
)
